...
public abstract class CacheRestControllerTemplate {
	...
	protected static final CCacheStatsOrderBy DEFAULT_ORDER_BY =
			CCacheStatsOrderBy.builder().descending(CCacheStatsOrderBy.Field.size).build();
	...
	@PostMapping("/reset")
	public final JsonCacheResetResponse reset(
			@RequestBody @NonNull final JsonCacheResetRequest request) {
		assertAuth();
	...
	protected void resetAdditional(@NonNull final JsonCacheResetResponse response,
			@NonNull final JsonCacheResetRequest request) {}
	...
	public JsonCacheResetResponse resetForRecordId(
			@RequestParam("tableName") final String tableName,
			@RequestParam("recordId") final int recordId) {
		final JsonCacheResetResponse response = new JsonCacheResetResponse();
	...
	public JsonGetStatsResponse getStats(
			...
			@RequestParam(name = "orderBy",
					required = false) @Nullable final String orderByString) {
		assertAuth();
		return getStats(JsonCacheStatsQuery.builder().cacheName(cacheName).minSize(minSize)
				.labels(labels).orderByString(orderByString).build());
	}

	@PostMapping("/stats")
	public JsonGetStatsResponse getStats(
			@RequestBody @NonNull final JsonCacheStatsQuery jsonQuery) {
		assertAuth();

		final CCacheStatsPredicate filter = jsonQuery.toCCacheStatsPredicate();
		final CCacheStatsOrderBy orderBy =
				jsonQuery.toCCacheStatsOrderBy().orElse(DEFAULT_ORDER_BY);

		return cacheMgt.streamStats(filter).sorted(orderBy).map(JsonCacheStats::of)
				.collect(JsonGetStatsResponse.collect());
	}

	@GetMapping("/byId/{cacheId}")
	public JsonCache getById(@PathVariable("cacheId") final String cacheIdStr,
		...
		if (cacheInterface instanceof final CCache<?, ?> cache) {
			...
			throw new AdempiereException("Cache of type "
					+ cacheInterface.getClass().getSimpleName() + " is not supported");
		}
	}
	...
	public Set<String> getRemoteCacheInvalidationTableNames() {
		return cacheMgt.getTableNamesToBroadcast().stream().sorted()
				.collect(ImmutableSet.toImmutableSet());
	}

	@GetMapping("/remoteCacheInvalidationTableNames/add")
	public void enableRemoteCacheInvalidationForTableName(
			@RequestParam("tableName") final String tableName) {
