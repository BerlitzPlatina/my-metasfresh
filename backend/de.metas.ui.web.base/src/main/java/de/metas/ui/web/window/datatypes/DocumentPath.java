...
public final class DocumentPath {
	...
	public static DocumentPath rootDocumentPath(@NonNull final AdWindowId adWindowId,
			final int documentIdInt) {
	...
	public static DocumentPath rootDocumentPath(@NonNull final WindowId windowId,
			@NonNull final RepoIdAware documentRepoId) {
	...
	public static DocumentPath rootDocumentPath(final DocumentType documentType,
			final DocumentId documentTypeId, final DocumentId documentId) {
	...
	public static List<DocumentPath> rootDocumentPathsList(final WindowId windowId,
			final String documentIdsListStr) {
		if (documentIdsListStr == null || documentIdsListStr.isEmpty()) {
			return ImmutableList.of();
		}

		return DocumentIdsSelection.ofCommaSeparatedString(documentIdsListStr).stream()
				.map(documentId -> rootDocumentPath(windowId, documentId))
				.collect(GuavaCollectors.toImmutableList());
	}

	public static DocumentPath includedDocumentPath(@NonNull final WindowId windowId,
			final String idStr, final String detailId, final String rowIdStr) {
		...
		return builder().setDocumentType(windowId).setDocumentId(idStr).setDetailId(detailId)
				.setRowId(rowIdStr).build();
	}

	public static DocumentPath includedDocumentPath(@NonNull final WindowId windowId,
			@NonNull final DocumentId documentId, @NonNull final DetailId detailId,
			@NonNull final DocumentId rowId) {
		return builder().setDocumentType(windowId).setDocumentId(documentId).setDetailId(detailId)
				.setRowId(rowId).build();
	}
	...
	public static DocumentPath includedDocumentPath(@NonNull final WindowId windowId,
		...
		return builder().setDocumentType(windowId).setDocumentId(documentId).setDetailId(detailId)
				.setRowIds(rowIds).build();
	}

	public static DocumentPath includedDocumentPath(@NonNull final WindowId windowId,
			@NonNull final DocumentId documentId, @NonNull final DetailId detailId) {
		return builder().setDocumentType(windowId).setDocumentId(documentId).setDetailId(detailId)
				.allowNullRowId().build();
	}

	/**
	 * Creates the path of a single document (root document or included document).
	 */
	public static DocumentPath singleWindowDocumentPath(@NonNull final WindowId windowId,
			final DocumentId id, final DetailId detailId, final DocumentId rowId) {
		return builder().setDocumentType(windowId).setDocumentId(id).setDetailId(detailId)
				.setRowId(rowId).build();
	}
	 ...
	 */
	private DocumentPath(final DocumentType documentType, final DocumentId documentTypeId,
	 ...
	 */
	private DocumentPath(@NonNull final DocumentType documentType,
			@NonNull final DocumentId documentTypeId, final DocumentId documentId,
			final DetailId detailId, @NonNull final DocumentIdsSelection rowIds) {
		this.documentType = documentType;
		this.documentTypeId = documentTypeId;
		this.documentId = documentId;
	 ...
	 */
	private DocumentPath(final DocumentType documentType, final DocumentId documentTypeId,
			final DocumentId documentId, final DetailId detailId, final DocumentId singleRowId) {
		super();
	...
	public String toString() {
		if (_toString == null) {
			...
			sb.append("/").append(documentType.getSymbol()).append(documentTypeId).append("/")
					.append(documentId);
			if (detailId != null) {
	...
	public boolean equals(final Object obj) {
		...
		return Objects.equals(documentType, other.documentType)
				...
				&& Objects.equals(detailId, other.detailId) && Objects.equals(rowIds, other.rowIds);
	}
	...
	public WindowId getWindowId() {
		Check.assume(documentType == DocumentType.Window, "Expect documentType={} for {}",
				DocumentType.Window, this);
	 ...
	 * @see {@link #isSingleNewIncludedDocument()} - to check if this is an included new document
	 *      please use
	 */
	...
	public boolean isComposedKey() {
		if (detailId == null) {
			...
			return rowIds != null && rowIds.isSingleDocumentId()
	...
	public DocumentPath withDocumentId(final DocumentId id) {
		if (isRootDocument()) {
			...
			if (Objects.equals(rowId, rowIdNew)) {
				...
				return new DocumentPath(documentType, documentTypeId, documentId, detailId,
						rowIdNew);
			}
		}
	}
	...
	public static final class Builder {
		...
		public DocumentPath build() {
			...
			if (documentTypeId == null) {
				throw new InvalidDocumentPathException(
			...
			if (!rowId_allowNull && detailId != null && rowIds.isEmpty()) {
				throw new InvalidDocumentPathException(
						"rowId cannot be null when detailId=" + detailId);
			}
			if (!rowId_allowNew) {
			...
			if (detailId == null && !rowIds.isEmpty()) {
				throw new InvalidDocumentPathException(
						"detailId cannot be null when we have rowIds: " + rowIds);
			}
		...
		public Builder setDocumentType(@NonNull final DocumentType documentType,
