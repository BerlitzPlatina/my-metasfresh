...
public class DocumentChangesCollector implements IDocumentChangesCollector {
	...
	private DocumentChanges rootDocumentChanges(final DocumentPath rootDocumentPath) {
		Check.assume(rootDocumentPath.isRootDocument(), "{} is root document path",
				rootDocumentPath);
	...
	public Stream<DocumentChanges> streamOrderedDocumentChanges() {
		return documentChangesByPath.values().stream()
				// skip document changes which are staled because it might be those were recorded
				// before the detailId was marked as stale
				.filter(documentChanges -> !isStaleDocumentChanges(documentChanges))
				.sorted(Comparator
						.comparing(documentChanges -> documentChanges.isPrimaryChange() ? 0 : 1)) // make
																									// sure
																									// primary
																									// changes
																									// are
	...
	@Override
	public void collectValueChanged(final IDocumentFieldView documentField,
			final ReasonSupplier reason) {
		documentChanges(documentField).collectValueChanged(documentField, reason);
	}

	@Override
	public void collectValueIfChanged(final IDocumentFieldView documentField, final Object valueOld,
			final ReasonSupplier reason) {
	...
	@Override
	public void collectReadonlyIfChanged(final IDocumentFieldView documentField,
			final LogicExpressionResult valueOld, final ReasonSupplier reason) {
		// If there is no change, don't collect the value
	...
	@Override
	public void collectMandatoryIfChanged(final IDocumentFieldView documentField,
			final LogicExpressionResult valueOld, final ReasonSupplier reason) {
		// If there is no change, don't collect the value
	...
	@Override
	public void collectDisplayedIfChanged(final IDocumentFieldView documentField,
			final LogicExpressionResult valueOld, final ReasonSupplier reason) {
		// If there is no change, don't collect the value
	...
	@Override
	public void collectLookupValuesStaled(final IDocumentFieldView documentField,
			final ReasonSupplier reason) {
	...
	public void collectFrom(final IDocumentChangesCollector fromCollector) {
		fromCollector.streamOrderedDocumentChanges().forEach(this::collectFrom);
	}
	...
	@Override
	public void collectDocumentValidStatusChanged(final DocumentPath documentPath,
	...
	@Override
	public void collectDocumentSaveStatusChanged(final DocumentPath documentPath,
	...
	@Override
	public void collectAllowNew(final DocumentPath rootDocumentPath, final DetailId detailId,
	...
	@Override
	public void collectAllowDelete(final DocumentPath rootDocumentPath, final DetailId detailId,
			final LogicExpressionResult allowDelete) {
		rootDocumentChanges(rootDocumentPath).includedDetailInfo(detailId)
	...
	private boolean isStaleDocumentChanges(final DocumentChanges documentChanges) {
		...
		return documentChangesIfExists(rootDocumentPath).flatMap(
				rootDocumentChanges -> rootDocumentChanges.includedDetailInfoIfExists(detailId))
				.map(IncludedDetailInfo::isStale).orElse(false);
	}
	...
	@Override
	public void collectFieldWarning(final IDocumentFieldView documentField,
