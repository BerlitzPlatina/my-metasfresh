...
public class RunMigrationScriptsTest {
	...
	@Disabled("Disabled for now because we don't want to run DB container in test")
	@Test
	void test() {
		Assumptions.assumeFalse(isSkipTest(),
				"`" + ENV_SKIP_MIGRATION_SCRIPTS_TEST + "` system property/env is not set");
	...
	private static GenericContainer<?> startDatabaseContainer(final DockerImageName imageName) {
		...
		final GenericContainer<?> db = new GenericContainer<>(imageName)
				// .withImagePullPolicy(PullPolicy.alwaysPull()) // needed then going with e.g.
				// "latest"
	...
	private static PropertyValue<File> getWorkspaceDir() throws IOException {
		...
		throw new AdempiereException("Cannot detect workspace directory."
				+ "\n Option 1. Pls make sure one of the ancestor of " + cwd + " contains `"
				+ DirectoryProperties.PROPERTIES_FILENAME + "` file."
	...
	private static class Config {
		@NonNull
		PropertyValue<File> workspaceDir;
		@NonNull
		PropertyValue<DockerImageName> dbImageName;
	}
}
