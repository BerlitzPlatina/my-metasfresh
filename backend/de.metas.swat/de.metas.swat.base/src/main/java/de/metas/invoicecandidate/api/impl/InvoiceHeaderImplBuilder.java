...
/**
 * {@link InvoiceHeaderImpl} builder class used to collect invoice header values
 * and make sure they match.
 * <p>
 * It creates the actual {@link InvoiceHeaderImpl} when calling
 * {@link #build()}.
 *
 * @author tsa
 */
...
public class InvoiceHeaderImplBuilder {
	...
	public InvoiceHeaderImpl build() {
		...
		invoiceHeader
				.setModularContractSettingsId(ModularContractSettingsId.ofRepoIdOrNull(getModularContractSettingsId()));
	...
	/**
	 * Set the header to the given {@code overrideDueDate}, if it is after a
	 ...
	 *         If different <code>M_PriceList_ID</code>s were added using
	 *         {@link #setM_PriceList_ID(int)},
	...
	/**
	 * Sets/adds the given pricelist for this header. See
	 * {@link #getM_PriceList_ID()} for how conflicts are resolved.
	 */
	...
	public void setBillTo(@NonNull final BPartnerInfo billTo) {
		if (this.billTo == null) {
			...
			if (!BPartnerInfo.equals(this.billTo.withLocationId(null).withContactId(null),
					billTo.withLocationId(null).withContactId(null))) {
				throw new AdempiereException("BillTo not matching: new=" + billTo + ", previous=" + this.billTo);
			}
		}

		if (this.billTo.getContactId() != null
				&& !BPartnerContactId.equals(billTo.getContactId(), this.billTo.getContactId())) {
	...
	public void setExternalId(@Nullable final String externalIdStr) {
		...
		Check.errorIf(ExternalId.isInvalid(externalId), "Given externalId may not be invalid"); // might happen later,
																								// when we modernize the
																								// method signature
	...
	public void setModularContractSettingsId(final int modularContractSettingsId) {
		this.MODCNTR_Settings_ID = checkOverrideID("MODCNTR_Settings_ID", this.MODCNTR_Settings_ID,
				modularContractSettingsId);
	}
	...
	public void setC_Tax_Departure_Country_ID(final int C_Tax_Departure_Country_ID) {
		this.C_Tax_Departure_Country_ID = checkOverrideID("C_Tax_Departure_Country_ID", this.C_Tax_Departure_Country_ID,
				C_Tax_Departure_Country_ID);
	}
